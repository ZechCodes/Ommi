...
@pytest.mark.asyncio
async def test_schema_evolution(driver: BaseDriver):
    """Test schema evolution capabilities.
    
    All drivers should support basic schema evolution:
    1. Adding new columns to existing tables
    2. Setting NULL as the default value for new columns on existing records
    
    Drivers should implement workarounds if the underlying database doesn't
    natively support these operations.
    """
    ...
    class EvolutionModel:
        ...
        value: float = None      # Another new field
        id: int = None

    try:
        # Apply evolved schema
        await driver.apply_schema(collection2)
        
        # Test 1: Verify old data is accessible with NULL values for new fields
        old_result = await driver.fetch(when(EvolutionModel.id == initial_model.id)).one()
        assert old_result.name == "Initial Item"
        assert old_result.description is None
        assert old_result.value is None
        
        # Test 2: Add new data with the new fields
        new_model = await driver.add([
            EvolutionModel(name="Evolution Item", description="New field", value=42.5)
        ])[0]
        
        # Verify new data with all fields
        new_result = await driver.fetch(when(EvolutionModel.id == new_model.id)).one()
        assert new_result.name == "Evolution Item"
        assert new_result.description == "New field"
        assert new_result.value == 42.5
        
        # Test 3: Update old record with values for the new fields
        await driver.update(
            when(EvolutionModel.id == initial_model.id),
            {"description": "Updated description", "value": 99.9}
        )
        
        updated_result = await driver.fetch(when(EvolutionModel.id == initial_model.id)).one()
        assert updated_result.name == "Initial Item"
        assert updated_result.description == "Updated description"
        assert updated_result.value == 99.9
    finally:
        # Clean up
